---
sidebar_position: 3
title: 快速上手
---

本页面快速概述了CommandPanels面板文件的工作原理、它们的主要部分，以及如何开始创建你自己的自定义界面。使用`/pa [参数]`开始主插件命令。

------

## 生成面板

你可以使用`/pa generate`命令从箱子的内容界面中快速创建一个新面板。

运行命令后：

- 你会进入**生成模式**。
- 在生成模式中，**右键任意一个箱子**。
- 就会通过箱子的布局和物品创建一个新的面板文件。

这种方法可以快速为箱子面板创建初始布局，无需手动编写YAML文件。

------

## 基础面板文件

- 每个面板都由`panels`文件夹中的YAML文件定义。

- 文件名**是面板的唯一名称**。

- 面板支持三种类型：

    - **`inventory`** — 传统箱子界面。
    - **`dialog`** — Java版玩家特有的对话框菜单。
    - **`floodgate`** — 基岩版玩家专属。

每种类型的配置选项都略有不同，但核心结构是相似的。

------

## 主面板属性

- **`type`**：面板类型（`inventory`，`floodgate`或`dialog`）。
- **`title`**：窗口的标题，可以使用占位符。
- **`conditions`**：指定面板是否可以打开的逻辑。
- **`command`**：打开面板的命令，可选带参数。
- **`commands`**：面板打开前要执行的命令。
- **`update-delay`**：定义箱子面板的更新频率（以刻为单位，0表示禁用更新）。
- **`rows`**：行数（仅箱子面板可用）。
- **`layout`**：定义物品在箱子面板中显示的槽位，以及对话框和基岩面板中组件的显示顺序。
- **`items`**：定义箱子面板中显示的物品，或对话框和基岩面板的组件。

------

## 示例面板文件

### 示例箱子面板

简易面板，一个物品在一个槽位中，点击可以发送消息。

```yaml
title: '箱子面板模板'
type: 'inventory'

rows: 1

layout:
  fill:
    - 'example_empty_item'
  4:
    - 'example_item'

items:
  example_empty_item:
    material: 'BLACK_STAINED_GLASS_PANE'

  example_item:
    material: 'REDSTONE'
    name: '示例物品'
    animate: 'frame_1'
    left-click:
      commands:
        - '[msg] 你点了我一下。'
```

### 示例对话框面板

为Java版玩家显示一个自定义箱子面板。

```yaml
title: '对话框面板模板'
type: 'dialog'

columns: 1
has-exit-button: false
escapable: true

layout:
  0:
    - 'welcome_text'
  1:
    - 'confirm_button'

items:
  welcome_text:
    type: 'text'
    name: '&6欢迎来到对话框！'

  confirm_button:
    type: 'button'
    name: '点击确认'
    actions:
      commands:
        - '[msg] 感谢点击！'
```

### 示例简易基岩面板

只能由基岩版玩家打开，一个用于按钮的简易基岩版表单。

```yaml
title: '基岩面板模板'
type: 'floodgate'

floodgate-type: 'simple'

layout:
  0:
    - 'example_button'

items:
  example_button:
    name: '点我！'
    icon:
      type: 'PATH'
      texture: 'textures/items/redstone_dust.png'
    actions:
      commands:
        - '[msg] 你好！爱来自水闸门~'
```

### 示例自定义基岩面板

只能由基岩版玩家打开，一个用于输入的基岩版表单。

```yaml
title: '基岩面板模板'
type: 'floodgate'

floodgate-type: 'custom'

layout:
  0:
    - 'inputexample'
  1:
    - 'sliderexample'

items:
  inputexample:
    type: 'input'
    name: '下方的文本框'
    placeholder: '在这里输入'
    default: ''
    actions:
      commands:
        - '[msg] 你说了：%commandpanels_session_inputexample%'

  sliderexample:
    type: 'slider'
    name: '滑动滑块'
    min: 1
    max: 10
    step: 1
    default: 5
    actions:
      commands:
        - '[msg] 你选择了：%commandpanels_session_sliderexample%'
```

## 了解操作和条件

CommandPanels使用功能强大且灵活的**点击操作**系统，该系统适用于*所有面板类型*。理解操作和条件的原理是构建交互式和动态菜单的关键。有关这些功能的详细说明，请参阅相关文档页面。

------

### 点击操作

当玩家与组件交互时（例如点击按钮或物品），会使用点击操作。操作支持：

- **需求标签**：可选，命令必须符合需求才能运行。
- **命令标签**：要运行的命令，例如发送消息、执行服务器命令或是打开其他面板。

通过以上即可实现**条件执行逻辑**。例如，玩家不符合需求时发送一条消息，符合需求，则打开另一个面板。

------

### 条件

**条件**控制可见性和行为。可用于：

- **面板**：决定玩家是否可以打开面板。
- **组件**：显示或隐藏指定的物品或按钮。

条件包括逻辑、占位符比较、权限、所处世界检查等。因此，面板能够根据玩家、世界或服务器状态动态调整，为每位玩家提供高度个性化的体验。

------

### 总结

| 功能       | 目的              | 使用场景         |
|----------|-----------------|--------------|
| **命令标签** | 执行不同操作的特殊命令     | 命令操作内部       |
| **需求标签** | 可选，命令必须符合需求才能运行 | 命令操作内部       |
| **条件**   | 控制组件的逻辑和面板的可见性  | 适用于所有面板组件和物品 |

------

## 提示

- 使用占位符可以让你的面板动态起来。
- 将物品和槽位分开管理，便于维护。
- 利用条件来根据每个玩家或上下文的情况自定义逻辑。
- 尝试使用动画帧来创建视觉反馈。
- 这应该能助你快速创建你的第一个面板！